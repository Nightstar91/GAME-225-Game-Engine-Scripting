...
{
    [SerializeField] private TextMeshProUGUI pegLabel;
    ...
    {
        // Keep the currentPeg in range of 1-3 to be use for DisplayCurrentPeg()
        if(1 <= currentPeg &&  currentPeg < 3)
        {
            currentPeg++;
        }
    }
    ...
    void CheckWinState()
    {
        // Checking the entire array of the third peg to see if the space isn't occuped with 0
        if(peg3[0] != 0 && peg3[1] != 0 && peg3[2] != 0 && peg3[3] != 0)
        {
            // Indicate a win to the player
            pegLabel.text = string.Format("You have solve the tower!");
        }
    }


    // Display whats the current peg to the player
    void DisplayCurrentPeg()
    {
        pegLabel.text = string.Format($"Currently at Peg {currentPeg}");
    }
